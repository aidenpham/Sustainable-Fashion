...
apollo_beta=c(c=0, asc_opt_out=0,
              ...
              b_Gender=0, b_Residence=0, b_Clothes=0, b_JeansFreq=0, 
              b_EnvLabor=0, b_GenderPrice=0, b_EducationEnv=0, 
              b_ResidenceEnv=0, b_ResidencePrice=0, b_ConcernSwtLabor=0,
              b_SkLabor=0)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V = list() 
    V[['alt1']]  = c + b_Price*Price1 + b_Origin*Origin1 + b_Fiber*Fiber1 +
                    ...
                    b_Gender*Gender + b_Residence*Residence + b_Clothes*Clothes + b_JeansFreq*JeansFreq +
      Price1*(Gender*b_GenderPrice + Residence*b_ResidencePrice) +
      log(Env1)*(Education*b_EducationEnv + Residence*b_ResidenceEnv) +
      log(Labor1)*(log(Env1)*b_EnvLabor + ConcernSwt*b_ConcernSwtLabor + Skepticism*b_SkLabor)
    ...
    V[['alt2']]  = c + b_Price*Price2 + b_Origin*Origin2 + b_Fiber*Fiber2 +
      ...
      log(Env2)*(Education*b_EducationEnv + Residence*b_ResidenceEnv) +
      log(Labor2)*(log(Env2)*b_EnvLabor + ConcernSwt*b_ConcernSwtLabor + Skepticism*b_SkLabor)
...
df = read.csv("MNL Combined5_estimates.csv",header=TRUE,sep=",")
library(writexl)
write_xlsx(df,"MNL Combined5_estimates.xlsx")
